...
const getCostumerData = async (page = 1) => {
    axios
        .get(`/api/invitationlinks?page=${page}`, {
            params: {
...
const activateUserData = () => {
    ...
    axios
        .put(`/api/invitationlinks/${dataIdBeingUsed.value}/activate`)
        .then((response) => {
...
const deleteData = () => {
    ...
    axios
        .delete(`/api/invitationlinks/${dataIdBeingUsed.value}`)
        .then(() => {
...
const debouncedinvitationlinksearch = debounce(() => {
    getCostumerData(currentCostumerPage.value);
}, 300);


watch(searchCostumerQuery, debouncedinvitationlinksearch);
                ...
                >
                    <template #header>
                        <div class="flex justify-between flex-col sm:flex-row">
                            <router-link to="/admin/invitationlinks/create">
                    ...
                    <Column header="Empresa" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.company ? data.company.name : 'N/A' }}
                        </template>
                    </Column>
                    ...
                    <Column header="Estado" style="min-width: 5rem">
                        <template #body="{ data }">
                            <Tag :value="data.is_active === 1 ? 'Ativo' : 'Inativo'" :severity="getSeverityStatus(data.is_active)" />
                        </template>
                    </Column>
                    <Column header="Ações" style="min-width: 12rem">
                        <template #body="{ data }">
                            <router-link
                                class="m-3"
                                :to="'/admin/invitationlinks/' + data.id + '/edit'"
                                ><i class="pi pi-file-edit"></i
                            ></router-link>
                            <router-link class="m-3" :to="'/admin/invitationlinks/' + data.id"
                                ><i class="pi pi-eye"></i
                            ></router-link>
                            <!-- <a
                                class="m-3"
